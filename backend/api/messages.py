...
async def update_message(
    ...
    # 返回更新后的消息
    messages, _ = message_service.get_user_messages(current_user.id, page=1, size=1)
    updated_message = next((msg for msg in messages if msg.id == message_id), None)
    
    if not updated_message:
        raise HTTPException(status_code=404, detail="消息不存在")
    
    return ApiResponse(data=MessageResponse.model_validate(updated_message))
...
async def create_message_action(
    ...
    # 验证消息是否属于当前用户
    messages, _ = message_service.get_user_messages(current_user.id, page=1, size=1000)
