...
def test_search_users(token, search_term):
    """测试用户搜索功能"""
    print(f"\n🔍 测试用户搜索功能 (搜索: {search_term})...")
    ...
    print(f"搜索请求: GET {BASE_URL}/families/search-users?q={search_term}")
    print(f"响应状态码: {response.status_code}")
...
def main():
    ...
    # 2. 注册额外的测试用户
    registered_users = register_additional_users()
    ...
    # 4. 测试用户搜索功能（使用已注册的用户名）
    if registered_users:
        # 搜索第一个注册的用户
        first_user = registered_users[0]
        search_results = test_search_users(token, first_user["username"])
        
        # 如果搜索不到，尝试搜索部分用户名
        if not search_results:
            print("\n🔍 尝试搜索部分用户名...")
            search_results = test_search_users(token, "test_user_invite")
        
        # 如果还是搜索不到，直接使用注册的用户进行测试
        if not search_results:
            print("\n⚠️  搜索功能返回空结果，直接使用注册的用户进行测试")
            search_results = registered_users
    
    # 5. 测试创建家庭并邀请成员
    if registered_users:
        family_id = test_create_family_with_invite(token, registered_users)
        if family_id:
            # 6. 获取家庭成员列表
    ...
    print("\n📝 测试总结:")
    print("- 用户注册功能正常")
    print("- 用户搜索功能可能存在问题（返回空结果）")
    print("- 创建家庭并邀请成员功能需要进一步测试")
    print("- 建议检查搜索逻辑和数据库查询")
